...
function ConfirmEmailContent() {
  ...
  const handleResend = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result?.message ?? 'Ett fel inträffade när mailet skulle skickas på nytt.');
      }

      setResendStatus(
        result?.message ?? 'Om adressen är registrerad skickas snart ett nytt bekräftelsemail.'
      );
    } catch (error) {
      setResendError(
